...
export class MainMsgSabChannel {
    ...
    constructor(sabOut: SharedArrayBuffer, sabIn: SharedArrayBuffer, worker: Worker) {
    ...
    private pumpOnce(): boolean {
        ...
        while (cursor < end) {
            ...
            const m = out.msg;
            if (m) {
                try { this.dispatch(m, this.worker); } catch { /* handler already guarded */ }
            }
        }
